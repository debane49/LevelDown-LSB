...
-----------------------------------
local m = Module:new('trion')

local trustToReplaceName = 'trion'
...
m:addOverride(string.format('xi.actions.spells.trust.%s.onMobSpawn', trustToReplaceName), function(mob)
    ...
    mob:addSimpleGambit(ai.t.TARGET, ai.c.ALWAYS, 0, ai.r.JA, ai.s.SPECIFIC, xi.ja.SHIELD_BASH) -- Shield Bash
    mob:addSimpleGambit(ai.t.TARGET, ai.c.ALWAYS, 0, ai.r.MS, ai.s.SPECIFIC, 3193, 60)          -- Royal Bash
    ...
    mob:addSimpleGambit(ai.t.SELF, ai.c.NOT_STATUS, xi.effect.SENTINEL, ai.r.MS, ai.s.SPECIFIC, 3194, 120)               -- Uses Royal Savior when sentinel wears
