...
--]]
    ...
    [xi.zone.SEALIONS_DEN] =
    {
        ...
        { 2,  994,    0 },   -- *The Warrior's Path (HTMBF)
        { 3,  995,    0 },   -- *One to Be Feared (HTMBF)
    },
    ...
    [xi.zone.EMPYREAL_PARADOX] =
    {
        ...
        { 2, 1058,    0 },   -- Both Paths Taken (ROVM2-9-2)
        { 3, 1059,    0 },   -- *Dawn (HTMBF)
    ...
    --  { 2, 1090,    0 },   -- Puppetmaster Blues (Quest)
        ...
        { 5, 1093,    0 },   -- *Legacy of the Lost (HTMBF)
    },
    ...
    --  { 3, 1155,    0 },   -- Moment of Truth (Quest)
        ...
        { 5, 1157,    0 },   -- *Puppet in Peril (HTMBF)
    },
    ...
    --  { 5,    ?, 4063 },   -- *Kindred Spirits (SKC20)
        { 6,  166,    0 },   -- *The Shadowlord Battle (HTMBF)
    --  { 7,    ?,    0 },   -- True Love
    --  { 8,    ?,    0 },   -- A Fond Farewell
    },
    ...
    --  { 3,  227,    0 },   -- Battaru Royale (ASA10)
        { 4,  228,    0 },   -- *The Moonlit Path (HTMBF)
        { 5,  229,    0 },   -- *Waking the Beast (HTMBF)
    },
    ...
    --  { 6,  262,    0 },   -- Ode of Life Bestowing (ACP11)
        { 7,  263,    0 },   -- *Return to Delkfutt's Tower (HTMBF)
    --  { 8,    ?,    0 },   -- True Love
    --  { 9,    ?,    0 },   -- A Fond Farewell
    },
    ...
    [xi.zone.LALOFF_AMPHITHEATER] =
    {
        ...
        {  6,  294,    0 },   -- *Ark Angels 1 (HTMBF)
        {  7,  295,    0 },   -- *Ark Angels 2 (HTMBF)
        {  8,  296,    0 },   -- *Ark Angels 3 (HTMBF)
        {  9,  297,    0 },   -- *Ark Angels 4 (HTMBF)
        { 10,  298,    0 },   -- *Ark Angels 5 (HTMBF)
        { 11,  299,    0 },   -- *Divine Might (HTMBF)
    },

    [xi.zone.THE_CELESTIAL_NEXUS] =
    {
        { 0,  320,    0 },   -- The Celestial Nexus (ZM16)
        { 1,  321,    0 },   -- *The Celestial Nexus (HTMBF)
    },
    ...
    --  { 3,  451,    0 },   -- Waking the Beast (Quest)
        ...
        { 5,  453,    0 },   -- *Trial by Lightning (HTMBF)
    },
    ...
    --  { 2,  610,    0 },   -- Waking the Beast (Quest)
        ...
        { 4,  612,    0 },   -- *Trial by Water (HTMBF)
    },

}
local m = Module:new("new_bcnm")
-----------------------------------
...
--  local acpMission       = player:getCurrentMission(xi.mission.log_id.ACP) NOTE: UNUSED Until BCNMID 532 is Re-enabled
    ...
    local registerReqs =
    {
        ...
        [1058] = function() -- Both Paths Taken (ROVM2-9-2)
            return player:getCurrentMission(xi.mission.log_id.ROV) == xi.mission.id.rov.BOTH_PATHS_TAKEN
        end,
        ...
        ----------------------NEW---------------------------------------------------------------
        [166] = function() -- 
            return player:hasKeyItem(xi.ki.SHADOW_LORD_PHANTOM_GEM)
        end,

        [263] = function() --
            return player:hasKeyItem(xi.ki.STELLAR_FULCRUM_PHANTOM_GEM)
        end,

        [294] = function() -- 
            return player:hasKeyItem(xi.ki.PHANTOM_GEM_OF_APATHY)
        end,

        [295] = function() -- 
            return player:hasKeyItem(xi.ki.PHANTOM_GEM_OF_COWARDICE)
        end,

        [296] = function() -- 
            return player:hasKeyItem(xi.ki.PHANTOM_GEM_OF_ENVY)
        end,

        [297] = function() -- 
            return player:hasKeyItem(xi.ki.PHANTOM_GEM_OF_ARROGANCE)
        end,

        [298] = function() -- 
            return player:hasKeyItem(xi.ki.PHANTOM_GEM_OF_RAGE)
        end,

        [299] = function() -- 
            return player:hasKeyItem(xi.ki.P_PERPETRATOR_PHANTOM_GEM)
        end,

        [321] = function() -- 
            return player:hasKeyItem(xi.ki.CELESTIAL_NEXUS_PHANTOM_GEM)
        end,

        [968] = function() -- 
            return player:hasKeyItem(xi.ki.SAVAGES_PHANTOM_GEM)
        end,

        [680] = function() -- 
            return player:hasKeyItem(xi.ki.HEAD_WIND_PHANTOM_GEM)
        end,

        [995] = function() -- 
            return player:hasKeyItem(xi.ki.FEARED_ONE_PHANTOM_GEM)
        end,

        [994] = function() -- 
            return player:hasKeyItem(xi.ki.WARRIORS_PATH_PHANTOM_GEM)
        end,

        [1059] = function() -- 
            return player:hasKeyItem(xi.ki.DAWN_PHANTOM_GEM)
        end,

        [1157] = function() -- 
            return player:hasKeyItem(xi.ki.PUPPET_IN_PERIL_PHANTOM_GEM)
        end,

        [1093] = function() -- 
            return player:hasKeyItem(xi.ki.LEGACY_PHANTOM_GEM)
        end,

        [548] = function() -- 
            return player:hasKeyItem(xi.ki.AVATAR_PHANTOM_GEM)
        end,
        ...
        [228] = function() -- 
            return player:hasKeyItem(xi.ki.MOONLIT_PATH_PHANTOM_GEM)
        end,

        [229] = function() -- 
            return player:hasKeyItem(xi.ki.WAKING_THE_BEAST_PHANTOM_GEM)
        end,

        [707] = function() -- 
            return player:hasKeyItem(xi.ki.WAKING_DREAMS_PHANTOM_GEM)
        end,

        [1088] = function() -- 
            return player:hasKeyItem(xi.ki.CONFIDENTIAL_IMPERIAL_ORDER)
        end,

        [1089] = function() -- 
            return player:hasKeyItem(xi.ki.SECRET_IMPERIAL_ORDER)
        end,
...
----------------------NEW---------------------------------------------------------------
        [166] = function() -- CELESTIAL_NEXUS_PHANTOM_GEM 
            return player:hasKeyItem(xi.ki.SHADOW_LORD_PHANTOM_GEM)
        end,
...
local function findBattlefieldz(player, npc, itemId)
    local mask = 0
    local zbfs = battlefieldz[player:getZoneID()]
...
m:addOverride('xi.bcnm.onTrade', function(player, npc, trade, onUpdate)
    ...
    local validBattlefieldz = findBattlefieldz(player, npc, itemId)

    if validBattlefieldz ~= 0 then
        if not onUpdate then
            player:startEvent(32000, 0, 0, 0, validBattlefieldz, 0, 0, 0, 0)
        end
...
m:addOverride('xi.bcnm.onTrigger', function(player, npc)
    -- Cannot enter if anyone in party is level/master sync'd
   -- player:PrintToPlayer('using new_bcnm')
    ...
    -- Player doesn't have battlefield status effect. That means player wants to register a new battlefield OR is attempting to enter a closed one.
        ...
        local mask = findBattlefieldz(player, npc, 0)
...
-- onEventUpdate
-----------------------------------
m:addOverride('xi.bcnm.onEventUpdate', function(player, csid, option, extras)
...
m:addOverride('xi.bcnm.onEventFinish', function(player, csid, option, npc)
